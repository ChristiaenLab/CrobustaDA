...
#' @param dat A list of objects coercible to a \code{data.frame}.
#' @param value The name to be assigned to the \code{'value'} column of a molten \code{data.frame}.
#' @export
#' 
...
#' @param conn A \code{SQLiteConnection}.
#' @param name The table name.
#' @param value The table to be written to \code{conn}. It should be coercible to a \code{data.frame}.
#' @param primary Either \code{FALSE} or the column to be used as the primary key.
#' @param foreign Either \code{FALSE} or the column name to be used as the foreign key.
#' @param row.names Whether to write \code{row.names} to the table.
#' @param overwrite Whether to overwrite table \code{name} in \code{conn} if it already exists.
#' @param ... Additional arguments to \code{link{RSQLite::dbWriteTable()}}.
#' @export
#' @import RSQLite
dbWriteKey <- function(conn,name,value,primary=F,foreign=F,row.names=F,overwrite=T,...){
  ...
  if(is.character(row.names)){
    ...
    value <- do.call(
	function(...) data.frame(..., stringsAsFactors=F),
	append(list(row.names(value)),value)
    )
  ...
  dbWriteTable(conn,name,value,...,append=T)#,row.names=row.names)
}

#' Wrapper for \code{dbWriteKey(..., foreign = 'gene_name(GeneID)')}
#' @export
dbWriteGenes <- function(...) dbWriteKey(...,foreign = 'gene_name(GeneID)')

#' Wrapper for \code{dbWriteKey(..., foreign = 'gene_name(KHID)')}
#' @export
dbWriteKHID <- function(...) dbWriteKey(...,foreign = 'gene_name(KHID)')

#' Wrapper for \code{dbWriteKey(..., foreign = 'gene_name(KYID)')}
#' @export
dbWriteKYID <- function(...) dbWriteKey(...,foreign = 'gene_name(KYID)')

#' Wrapper for \code{dbWriteKey(..., foreign = 'peakfeature(PeakID)')}
#' @export
...
#' @param conn A SQLiteConnection.
#' @param name The name of the table.
#' @param value The table to be added to \code{conn}.
#' @param row.names The column name to be used for \code{row.names(value)}.
#' @param ... Additional arguments to \code{\link{dbWriteTable}}.
#' @export
...
#' Retrieves significant rows from a table for a specified FDR and log2FoldChange.
#' @param conn A SQLiteConnection.
#' @param table The name of the table to select from.
#' @param lfc A string specifying the log2FoldChange value to select. It should be in the format \code{'>1'} or \code{'<-0.75'}.
#' @param p The FDR value to select.
#' @export
getSig <- function(conn,table,lfc,p=0.05) {
	#   require(DBI)
...
#' Retrieves significant rows from a table for a specified FDR and log2FoldChange. As \code{link{getSig()}}, but for absolute values of log2FoldChange.
#' @param conn A SQLiteConnection.
#' @param table The name of the table to select from.
#' @param lfc The log2FoldChange value to select. 
#' @param p The FDR value to select.
#' @export
#' @seealso \code{\link{getSig}}
...
#' accepts a vector of KYIDs
#' returns a data.frame of all peaks associated to genes
#' @param con A SQLiteConnection.
#' @param genes A vector of KYIDs.
#' @export
geneToPeak <- function(con,genes){
	#   require(DBI)
  return(dbGetQuery(con,paste0(
    'SELECT KYID, PeakID FROM geneToPeak WHERE KYID IN ("',
...
#' accepts a vector of KYIDs
#' returns a data.frame of all KHIDs associated to KYIDs
#' @param con A SQLiteConnection.
#' @param genes A vector of KYIDs.
#' @export
kyToKh <- function(con,genes){
	#   require(DBI)
  return(dbGetQuery(con,paste0(
    'SELECT KYID, KHID FROM KH_KY WHERE KYID IN ("',
    paste(genes,collapse = '", "'),
    '")'
  )))
}

#' accepts a vector of KHIDs
#' returns a data.frame of all KYIDs associated to KHIDs
#' @param con A SQLiteConnection.
#' @param genes A vector of KHIDs.
#' @export
khToKy <- function(con,genes){
	#   require(DBI)
  return(dbGetQuery(con,paste0(
    'SELECT KYID, KHID FROM KH_KY WHERE KHID IN ("',
    paste(genes,collapse = '", "'),
    '")'
  )))
}
...
#' @param field The field name of \code{table} used to split it into a list.
#' @param select A string specifying other fields to select from \code{table}.
#' @param ... Additional arguments to \code{\link{dbGetQuery()}}.
#' @seealso \code{\link{dbGetQuery}}
#' @export
...
#' returns a data.frame of all KYIDs associated to peaks
#' @param con A SQLiteConnection.
#' @param peaks A vector of PeakIDs.
#' @export
peakToGene <- function(con,peaks){
	#   require(DBI)
  return(dbGetQuery(con,paste0(
    'SELECT PeakID, KYID FROM geneToPeak WHERE PeakID IN("',
    paste(peaks,collapse = '","'),
    '")'
  )))
}

#' accepts a database connection, list of KYIDs, and list of PeakIDs
...
#' as \code{\link{mergeGenePeak()}}, but also returns gene names and peak coordinates
#' @param con A SQLiteConnection.
#' @param genes A vector of GeneIDs.
#' @param peaks A vector of PeakIDs.
#' @seealso \code{\link{mergeGenePeak}}
#' @export
...
#' @param con A SQLiteConnection.
#' @param lib A vector of ATAC-seq comparisons corresponding to the levels of the \code{comparison} field of the \code{atacseq} table.
#' @export
